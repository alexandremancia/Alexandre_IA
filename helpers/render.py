...
try:
    from captions_presets import (
        get_preset as get_caption_preset,
        force_style_for as caption_force_style_for,
        DEFAULT_PRESET as DEFAULT_CAPTION_PRESET,
    )
except Exception:
    def get_caption_preset(name: str) -> dict:  # type: ignore
        return {"max_words": 2, "uppercase": True}

    def caption_force_style_for(name: str, format_target=None) -> str:  # type: ignore
        return ""
    DEFAULT_CAPTION_PRESET = "natural-sentence"

try:
    from format_targets import build_reframe_filter
except Exception:
    def build_reframe_filter(target: str, strategy: str = "blur-pad") -> str:  # type: ignore
        raise RuntimeError("format_targets.py not found — cannot reframe")
...
def build_master_srt(
    ...
    - `max_words`-word chunks (break early on any punctuation in between)
    - UPPERCASE text when `uppercase` (defaults preserve the original
      2-word/uppercase behavior when called with no extra args — the
      caption-style preset picked in main() supplies real values)
    ...
    for r in edl["ranges"]:
        ...
        for w in words_in_seg:
            ...
            if len(current) >= max_words or ends_in_punct:
        ...
        for chunk in chunks:
            ...
            # Strip trailing punctuation for a cleaner look either way
            text = text.rstrip(",;:")
            if uppercase:
                text = text.upper()
...
def build_final_composite(
    ...
    """Final pass: base → [reframe] → overlays (PTS-shifted) → subtitles LAST → out.

    `reframe_filter`, when given (a --format target was requested), runs
    FIRST — before overlays and subtitles — so a vertical crop or blur-pad
    doesn't clip graphics or captions that were positioned for the original
    frame. Subtitles still apply LAST regardless — that ordering is a hard
    rule, not something --format is allowed to change.

    If there is nothing to do at all (no reframe, no overlays, no subtitles),
    just copy base to out.
    """
    has_overlays = bool(overlays)
    has_subs = subtitles_path is not None and subtitles_path.exists()
    has_reframe = bool(reframe_filter)

    if not has_overlays and not has_subs and not has_reframe:
    ...
    # Reframe FIRST, on the base video stream only.
    current = "[0:v]"
    if has_reframe:
        filter_parts.append(f"{current}{reframe_filter}[reframed]")
        current = "[reframed]"
    ...
    # Chain overlays on top of the (possibly reframed) base
    for idx, ov in enumerate(overlays, start=1):
        t = float(ov["start_in_output"])
    ...
    if has_subs:
        ...
        out_label = "[outv]"
    elif has_overlays or has_reframe:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Named preset from captions_presets.py (e.g. bold-overlay, natural-sentence, "
             "minimal-clean, karaoke-word). Default preserves the original 2-word/uppercase "
             "bold style for exact backward compatibility.",
    )
    ap.add_argument(
        "--format",
        type=str,
        default=None,
        help="Reframe target from format_targets.py (e.g. vertical-reel, horizontal-youtube, "
             "square-feed). Omit to keep the source's native aspect ratio.",
    )
    ap.add_argument(
        "--reframe-strategy",
        type=str,
        ...
        help="How --format fills a mismatched aspect ratio. Default blur-pad (blurred, "
             "zoomed background fills the bars — looks less 'repurposed' than a hard crop).",
    )
    ap.add_argument(
        "--no-loudnorm",
        action="store_true",
    ...
    # 2b. Caption style: resolve the preset once, used both for chunking (SRT)
    # and for the burn-in force_style (composite step).
    if args.caption_style:
        caption_cfg = get_caption_preset(args.caption_style)
        srt_max_words, srt_uppercase = caption_cfg["max_words"], caption_cfg["uppercase"]
        # MarginV depends on the delivery format (vertical platforms overlay
        # the bottom ~30% with UI), so the style is built from the preset AND
        # the --format target, not read as a fixed string.
        force_style = caption_force_style_for(args.caption_style, args.format) or SUB_FORCE_STYLE
    else:
        # No --caption-style passed: exact original behavior (2-word, uppercase, SUB_FORCE_STYLE)
        srt_max_words, srt_uppercase, force_style = 2, True, SUB_FORCE_STYLE
    ...
    if not args.no_subtitles:
        if args.build_subtitles:
            ...
            build_master_srt(edl, edit_dir, subs_path, max_words=srt_max_words, uppercase=srt_uppercase)
    ...
    reframe_filter: str | None = None
    if args.format:
        reframe_filter = build_reframe_filter(args.format, args.reframe_strategy)
        print(f"reframing → {args.format} ({args.reframe_strategy})")

    # 4. Composite (reframe → overlays → subtitles LAST) → intermediate (pre-loudnorm) path
    overlays = edl.get("overlays") or []
    if args.no_loudnorm:
        # Composite directly to final output
        build_final_composite(base_path, overlays, subs_path, out_path, edit_dir,
                               force_style=force_style, reframe_filter=reframe_filter)
    else:
