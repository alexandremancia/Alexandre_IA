...
from transcribe import load_api_key_optional, transcribe_one, transcript_path
...
def main() -> None:
    ...
    ap.add_argument("--engine", default="auto", choices=["auto", "scribe", "local"],
                    help="auto = Scribe se houver chave, senão Whisper local")
    ap.add_argument("--whisper-model", default="base",
                    choices=["tiny", "base", "small", "medium", "large-v3"],
                    help="Modelo do faster-whisper no caminho local")
    ap.add_argument(
        "--language",
        type=str,
        default=None,
    ...
    if not api_key and args.engine != "scribe":
        # Whisper local em 4 threads compete por CPU; serializa para não travar a máquina.
        if args.workers > 2:
            print("  transcrição local: reduzindo para 2 workers (CPU-bound)")
            args.workers = 2
