...
PRESETS: dict[str, str] = {
    ...
    # --- ManciaSolutions: presets adicionais -------------------------------
    # Redes sociais: satura e contrasta o suficiente para sobreviver à
    # recompressão agressiva de TikTok/Reels sem virar desenho.
    "vivid_social": (
        "eq=contrast=1.10:saturation=1.18,"
        "unsharp=5:5:0.6:5:5:0.0,"
        "curves=master='0/0 0.2/0.18 0.8/0.84 1/1'"
    ),

    # Pele em primeiro lugar: contraste e nitidez sem puxar vermelho nem
    # saturar o rosto. Padrão seguro para talking head e entrevista.
    "skin_safe": (
        "eq=contrast=1.05:saturation=1.02,"
        "colorbalance=rm=-0.01:gm=0.01:bm=0.0,"
        "curves=master='0/0 0.25/0.24 0.75/0.77 1/1'"
    ),

    # Log/flat: achata para regravar por cima com LUT ou grade externa.
    # Não é um "look" — é ponto de partida para colorização manual.
    "flat_log": (
        "eq=contrast=0.88:saturation=0.82:brightness=0.04,"
        "curves=master='0/0.06 0.5/0.5 1/0.94'"
    ),

    # Frio/técnico: produto, software, B2B. Highlights levemente azuis.
    "cool_tech": (
        "eq=contrast=1.08:saturation=0.94,"
        "colorbalance=rs=-0.02:bs=0.03:rh=-0.03:bh=0.05,"
        "curves=master='0/0 0.25/0.23 0.75/0.78 1/1'"
    ),

    # Preto e branco com contraste de filme.
    "bw_film": (
        "hue=s=0,eq=contrast=1.18,"
        "curves=master='0/0 0.2/0.14 0.8/0.88 1/1'"
    ),
...
def lut_filter(lut_path: str, interp: str = "tetrahedral") -> str:
    """Filtro lut3d para um .cube/.3dl.

    LUT criativa vai DEPOIS da correção técnica e ANTES do sharpen. Uma LUT
    aplicada sobre material já contrastado satura duas vezes e estoura a pele.
    """
    p = Path(lut_path)
    if not p.exists():
        raise FileNotFoundError(f"LUT não encontrada: {lut_path}")
    escaped = str(p.resolve()).replace("\\", "/").replace(":", "\\:")
    return f"lut3d=file='{escaped}':interp={interp}"
