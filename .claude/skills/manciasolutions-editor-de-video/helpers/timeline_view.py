...
def probe_duration(video: Path) -> float:
    """Duração em segundos, ou 0.0 se não der para ler."""
    proc = subprocess.run(
...
def extract_frames(video: Path, start: float, end: float, n: int, dest_dir: Path) -> list[Path]:
    ...
    # Buscar exatamente no fim do arquivo não devolve frame nenhum: o ffmpeg sai
    # com código 0 e não escreve nada. Sem este recuo, a última amostra de uma
    # janela que vai até o fim do clipe some e só estoura lá adiante, no PIL.
    last = probe_duration(video)
    if last > 0:
        limit = max(0.0, last - 0.05)
    ...
    for i, t in enumerate(times):
        ...
        # Cinto de segurança: um frame que não saiu não pode derrubar o resto.
        if out.exists():
            paths.append(out)
        else:
            missing += 1
    if missing:
        print(f"  aviso: {missing} frame(s) não puderam ser extraídos e foram pulados")
    return paths
